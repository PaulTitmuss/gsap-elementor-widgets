...
class Hero_Bento extends Widget_Base {
        ...
        protected function register_controls() {
                ...
                $this->add_control(
                        'video_url',
                        array(
                                'label'       => esc_html__( 'Video URL (MP4, YouTube or Vimeo)', 'gsap-elementor-widgets' ),
                                'description' => esc_html__( 'Paste a YouTube link (e.g. https://www.youtube.com/watch?v=… or https://youtu.be/…), a Vimeo link, or a direct .mp4 file link. It will autoplay muted and loop as the hero background.', 'gsap-elementor-widgets' ),
        ...
        protected function render() {
                ...
                if ( 'video' === $bg_type && ! empty( $settings['video_url'] ) ) {
                        $embed = self::get_video_embed_url( $settings['video_url'] );

                        if ( $embed ) {
                                // YouTube / Vimeo: use an iframe scaled to cover the hero.
                                echo '<div class="gsap-ew-herobento-bg gsap-ew-herobento-embed">';
                                echo '<iframe src="' . esc_url( $embed ) . '" frameborder="0" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen title="' . esc_attr__( 'Background video', 'gsap-elementor-widgets' ) . '"></iframe>';
                                echo '</div>';
                        } else {
                                // Direct self-hosted / external .mp4 file.
        ...
        /**
         * Turn a YouTube or Vimeo watch URL into an autoplaying, muted, looping
         * background embed URL. Returns an empty string for anything else (e.g. a
         * direct .mp4 link), so the caller falls back to a native <video> tag.
         *
         * @param string $url Raw URL pasted by the user.
         * @return string Embed URL, or '' if not a recognised provider.
         */
        private static function get_video_embed_url( $url ) {
                ...
                if ( $yt_id ) {
                        // playlist=<id> is required for a single video to loop on YouTube.
                        $args = array(
                                'autoplay'       => 1,
                                'mute'           => 1,
                                'controls'       => 0,
                        ...
                        return 'https://www.youtube.com/embed/' . $yt_id . '?' . http_build_query( $args );
                }
                ...
                if ( preg_match( '~vimeo\.com/(?:video/)?(\d+)~i', $url, $m ) ) {
                        ...
                        return 'https://player.vimeo.com/video/' . $vimeo_id . '?' . http_build_query( $args );
                }
